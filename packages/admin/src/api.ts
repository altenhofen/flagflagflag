...
const isPage = <T>(guard: (value: unknown) => value is T) => (value: unknown): value is Page<T> => object(value) && Array.isArray(value.data) && value.data.every(guard) && (value.pagination === undefined || (object(value.pagination) && (value.pagination.nextCursor === null || typeof value.pagination.nextCursor === 'string')));
...
export const api = {
  ...
  exportAudit: (projectId: string, filters: Pick<AuditFilters, 'environmentId' | 'resourceType' | 'action'> = {}) => { const query = new URLSearchParams(Object.entries(filters).filter((entry): entry is [string, string] => Boolean(entry[1]))); return request<string>(`/${path('projects', projectId, 'audit-logs')}/export${query.size ? `?${query}` : ''}`, { headers: { Accept: 'text/csv' } }); },
};
export { operators };
