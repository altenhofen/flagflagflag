...
export class AuditService {
  constructor(
    @Inject(AUDIT_REPOSITORY) private readonly repository: Repository<AuditEntryEntity>,
  ...
  async list(query: AuditQuery): Promise<{ data: AuditEntryEntity[]; nextCursor: string | null }> {
    ...
    const qb = this.repository.createQueryBuilder('audit')
      ...
      .take(limit + 1);
    if (query.environmentId) qb.andWhere('audit.environmentId = :environmentId', { environmentId: query.environmentId });
    ...
    if (query.cursor) {
      ...
      qb.andWhere('(audit.createdAt < :cursorDate OR (audit.createdAt = :cursorDate AND audit.id < :cursorId))', decoded);
    }
  ...
  async setRetention(retentionDays: number): Promise<{ retentionDays: number }> {
    if (!Number.isInteger(retentionDays) || retentionDays < 1 || retentionDays > 3650) {
      throw new BadRequestException('retentionDays must be an integer between 1 and 3650');
    }
...
function encodeCursor(entry: AuditEntryEntity): string {
  return Buffer.from(JSON.stringify({ date: entry.createdAt.toISOString(), id: entry.id })).toString('base64url');
}

function decodeCursor(cursor: string): { cursorDate: string; cursorId: string } {
  try {
...
function redact(value: Record<string, unknown> | null | undefined): Record<string, unknown> | null {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (/^(key|password|secret|token|privatekey|api[-_]?key)$/i.test(key) || /hash/i.test(key)) continue;
    output[key] = item;
  }
  return output;
}
