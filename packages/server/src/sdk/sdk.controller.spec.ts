...
describe('SDK transport', () => {
  it('authenticates X-SDK-Key header keys and attaches their environment', async () => {
    ...
    expect(sdk.authenticate).toHaveBeenCalledWith('staging-key');
    expect(request.sdkEnvironment).toEqual(environment);
  });

  it('rejects absent keys and returns ETags or 304 for config versions', async () => {
    ...
    await controller.config(
      { ...request, headers: { 'if-none-match': '"42"' } } as never,
      response as never,
    );
